                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-900">{formatDate(auction.registrationDeadline)}</p>
                    </div>
                  </div>
                </div>
              </div>
                      ...
                      {auction.bidLog && auction.bidLog.length > 0 ? (
                        <div className="divide-y divide-gray-200">
                          {[...auction.bidLog].reverse().map((bid, idx) => {
                            ...
                            return (
                              <div key={bid._id || idx} className={`flex justify-between items-center p-4 hover:bg-gray-100 transition-colors duration-150 ${isWinning ? 'bg-green-50 border-l-4 border-green-500' : ''}`}>
                                <div className="flex items-center gap-3">
                                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isWinning ? 'bg-green-100' : 'bg-blue-100'}`}>
                                    <span className={`text-xs font-semibold ${isWinning ? 'text-green-600' : 'text-blue-600'}`}>
                                      {bidderName[0].toUpperCase()}
                                    </span>
                                  </div>
                                  <div>
                                    <div className="flex items-center gap-2">
                                      <span className="font-medium text-gray-900">{bidderName}</span>
                                      {isWinning && (
